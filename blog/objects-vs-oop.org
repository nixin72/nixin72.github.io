#+TITLE: Using Objects != being Object Oriented
#+SETUPFILE: ../templates/html.org
#+INCLUDE: header.org

The term "object oriented programming" is something that we often misuse when talking about the use
of objects. The word "oriented" in particular is important, because it implies that an overall
system is geared towards the use of objects or that there's some interconnection and communication
between objects that makes our system oriented towards the use of objects. And I think that
this "oriented" part of object-oriented, that's the bad part.
...
/oriented/ part. We can introduce a small handful of objects to our system to manage things like
state, so long as we don't start having several objects communicating together or the scope of their
control is ill-defined or we allow them to grow into a fully object-oriented system in it's own right.
